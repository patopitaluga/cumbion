...
const findKeywords = (_wordIndex, _wordsCurrentLine, _wordsNextLine, _scopeStatus) => {
  ...
  if (currentWord === 'lxs') {
    ...
    if (
      ...
      !_scopeStatus.line.printing
    ) {
      _scopeStatus.line.willBePushedToArray++;
    }
  }
  ...
  // variable name
  if (isVariable(_wordIndex, _wordsCurrentLine, _scopeStatus)) {
    if (nextWord === 'de') {
      ...
      if (!isReserved(currentWord.toLowerCase())) {
        ...
        if (_wordIndex !== 0 && (_wordIndex !== 1 && prevWord !== 'lxs'))
          jsPartAccumulator += ' ';
        ...
        if (prevWord === 'lxs') {
          if (
            _scopeStatus.line.willBePushedToArray > 0 &&
            _scopeStatus.line.settingFunctionProps === 0 &&
            _scopeStatus.line.printing === 0 &&
            !_scopeStatus.line.inComparison
          )
  ...
  if (
    ...
    if (isVariable(_wordIndex + 1, _wordsCurrentLine, _scopeStatus)) {
      jsPartAccumulator += helpers.slugify(nextWord) + '++';
      _wordsCurrentLine[_wordIndex + 1] = ''; // avoid rendering later
    }
    if (secondNextWord && isVariable(_wordIndex + 2, _wordsCurrentLine, _scopeStatus)) {
  ...
  if (currentWord.toLowerCase() === 'baja') {
    ...
    if (isVariable(_wordIndex + 2, _wordsCurrentLine, _scopeStatus)) {
      jsPartAccumulator += helpers.slugify(secondNextWord) + '--';
      _wordsCurrentLine[_wordIndex + 1] = ''; // avoid rendering later
      _wordsCurrentLine[_wordIndex + 2] = ''; // avoid rendering later
    }
